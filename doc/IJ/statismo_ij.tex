...
\author{Marcel L\"uthi$^{1}$, R{\'e}mi Blanc$^{2}$, Thomas Albrecht$^{1}$, Tobias Gass$^{3}$, Orcun Goksel$^3$, Philippe B\"uchler$^{4}$, Michael Kistler$^4$, Habib Bousleiman$^4$,  Mauricio Reyes$^{4}$, Philippe C. Cattin$^{5}$, Thomas Vetter$^{1}$ }

\authoraddress{$^{1}$Department of Mathematics and Computer Science, University of Basel, Switzerland\\
               $^{2}$IMS Laboratory, University of Bordeaux, France\\
               $^{3}$Computer Vision Lab, ETH Zurich, Switzerland \\
               $^{4}$Institute for Surgical Technology and Biomechanics, University of Bern, Switzerland  \\
...
$2)$ To make the exchange of such models easier among different
research groups and to improve the reproducibility of the models.
$3)$ To allow for easy integration of new methods for model
...
possible to use \Statismo to create different kinds of PCA models,
...
of \Statismo with the ITK registration framework. This leverages the
efforts from the ITK project to readily access powerful methods for model
fitting.
...
These datasets are then added to the \code{DataManager}. Note that we also need to provide the path of the dataset as
...
\begin{figure}[tbh]
  ...
  of the femur the first component is not the size anymore (as in \ref{fig:vispca}), but shows mainly the variations of the femoral head and the condyles. }

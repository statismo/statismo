...
possible to use \Statismo to create all different kinds of PCA
models, but also allows \Statismo to be used with any toolkit and data format.  To facilitate
 data exchange, \Statismo defines a storage format based on
HDF5, which includes all the information necessary to use the model as
well as meta-data about the model creation, which increases the
reproducibility.  The last goal is achieved by providing a clear separation between data management, model building and model representation. 
In addition to the standard method for building PCA models, \Statismo already includes two recently proposed algorithms
building conditional models, as well as convenience tools for
facilitating cross-validation studies.
...
Assume for instance that \Statismo is used to build models from \code{itk::Mesh} types.

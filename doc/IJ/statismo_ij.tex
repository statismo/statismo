...
for \emph{STATistical Image and Shape MOdels}, is a framework for the
creation and exploration of PCA based statistical models.  Statistical models
are used to describe the variability of an object within a population,
learned from a set of training samples. These models have been developed
and become popular in the form of  shape models, but are now increasingly
used in different contexts - to learn the joint variability of shape
and appearance of objects, or to learn statistical deformation models.

\Statismo has been developed with the following main goals in mind: $1)$ To provide a
generic tools for learning all kinds of PCA based statistical models,
$2)$ to make the exchange of PCA models easier among different
research groups and $3)$ to allow for an easy integration of new
methods for model building into the framework.  The first goal has
been achieved by designing \Statismo to be independent of a specific
data representation.  This makes it possible to use the same methods
for building different kinds of statistical models. Furthermore, it
can be adapted to work with any toolkit and data format.
To allow for an easy data exchange and portability, \Statismo defines a storage format based on HDF5, which includes
all the information necessary to use the model and to ensure reproducibility.
The third goal is achieved by providing a clear separation between data management, model building 
and model representation. Besides the standard methods for building PCA models, \Statismo already includes two 
recently proposed algorithms for building conditional models, as well as convenience tools for facilitating 
cross-validation studies. 

Although \Statismo can be used together with any toolkit, special efforts have been made
for VTK and ITK. \Statismo directly
supports model creation from the most common data representations used by ITK and VTK. 
Furthermore, \Statismo provides an ITK transform class, which allows for the integration of \Statismo with the ITK
registration framework. This leverage the effort that has been done in
ITK, to obtain powerful methods for model fitting.
...
not strictly lie in this subspace is zero.  To see why this is a
...
of the object class represented by the model.  To alleviate this
...
Then, assuming that \Statismo is used to build models from \code{itk::Mesh} types, 
...
\subsection{Representers} \label{sec:representers} The A
\code{Representer} class has two distinct purposes: First, it is used
...
type of object that are modeled.  A \emph{Representer} class in
...
sample is a mesh, in the specific data representation used by the representer.
